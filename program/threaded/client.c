...
/* NOTE data_size is implied when we send strings, but not in general. */
/*      Where do we free values allocated by strdup (same below)? */
void client_set(char *key, char *data, size_t data_size) {
